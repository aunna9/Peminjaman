  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      try {
        ...
        // ✅ ini endpoint yang terbukti jalan di browser kamu
    ...
    <div className="admin-dashboard">
      <div className="header">
        <div>
          ...
          <p className="sub">
            Ringkasan pengelolaan user, inventaris alat, peminjaman, pengembalian, dan log aktivitas.
          </p>
        </div>
      ...
      <div className="grid">
        ...
        <div className="card">
          <div className="cardTop"><div className="cardLabel">Peminjaman Aktif</div></div>
          <div className="cardValue">{summary.peminjamanAktif}</div>
          <div className="cardHint">Dari /api/peminjaman</div>
        </div>

        <div className="card">
          <div className="cardTop"><div className="cardLabel">Menunggu Persetujuan</div></div>
          <div className="cardValue">{summary.menunggu}</div>
          <div className="cardHint">Dari /api/peminjaman</div>
        </div>

        <div className="card">
          <div className="cardTop"><div className="cardLabel">Terlambat</div></div>
      ...
      <div className="section">
        <div className="card">
          ...
          <table className="table">
            <thead>
              <tr>
                <th>Peminjam</th><th>Alat</th><th>Qty</th><th>Tanggal</th><th>Status</th>
              </tr>
            </thead>
            <tbody>
              {peminjamanTerbaru.length === 0 ? (
                <tr>
                  <td colSpan={5} style={{ padding: 12 }}>Belum ada data peminjaman.</td>
                </tr>
              ) : (
                peminjamanTerbaru.map((x, i) => (
                  <tr key={i}>
                    ...
                    <td>{x.qty}</td>
                    <td>{x.tgl}</td>
                    <td><span className={pillClass(x.status)}>{x.status}</span></td>
                  </tr>
                ))
              )}
            </tbody>
